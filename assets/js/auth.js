    ...
    try {
      ...
      if (data.user) {
        ...
        if (userData.role === "patient") {
          await this.createPatientProfile(data.user.id, userData);
        }
      }
  ...
  async createPatientProfile(userId, userData) {
    try {
      const patientData = {
        ...
        rh_factor: userData.rh_factor || null,
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
      };

      const { error } = await window.supabase.from("patients").insert([patientData]);

      if (error) throw error;
    } catch (error) {
      console.error("Error al crear perfil de paciente:", error.message);
      throw error;
    }
  }
